...
router.post("/phone/send-otp", async (req, res) => {
  try {
    ...
    // Check if user with phone exists (for login)
    const existingUser = await findUserByPhone(normalizedPhone);

    const result = await smsUtils.sendOTP(phone);
    ...
    res.json({
      ...
      phone: normalizedPhone.replace(/(\+\d{2})(\d{5})(\d+)/, "$1*****$3"),
      userExists: !!existingUser,
      // Include OTP in dev mode for testing
      ...(result.devMode && { devMode: true, otp: result.otp }),
    });
  } catch (error) {
...
router.post("/phone/verify-otp", async (req, res) => {
  try {
    ...
    // Verify OTP
    const verification = await smsUtils.verifyOTPCode(phone, otp);
...
async function findUserByPhone(phone) {
  const query =
    dbType === "postgres"
      ? "SELECT * FROM auth_users WHERE phone_number = $1 OR oauth_provider_id = $1"
      : "SELECT * FROM auth_users WHERE phone_number = ? OR oauth_provider_id = ?";

  if (dbType === "postgres") {
    const result = await db.query(query, [phone]);
    return result.rows[0];
  } else {
    return db.prepare(query).get(phone, phone);
  }
}
